...
\author{Audrey Seo} % Author(s)

\institute{REUSE -- ISR} % Institution(s)
...
\node[myinput,above=3in of sizet]			(inputs)	{apis: [[Node::make\_shared], [create\_subscription]]\\types: \{std::string: 2\}};

\path let \n1={-1.2in},
		  \p1=($(inputs.south)+(0,\n1)$) in
		 coordinate (myinputarrowend) at (\p1);

\node[trans,label=below left:{create\_subscription(A)}] at ($($(n)-(5,0)$)!(sizet)!(n)$)	(createsubA) {};
...
\node[myinput,below=1.4in of publish,align=left] (output) {shared\_ptr<Node> n = Node::make\_shared(????);\\n.create\_subscription(???,???,???);};
\path let \n1={1.2in},
		  \p1=($(output.north)+(0,\n1)$) in
...
\draw[flow] (stdstring) to [bend right=45] 				(createsubA);
\draw[flow] (ptrnode) 	to [bend left=45]				(derefnode);
\draw[flow] (sizet) to 									(createsubA);
